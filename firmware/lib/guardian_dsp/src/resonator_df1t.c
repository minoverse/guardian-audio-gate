...
int resonator_bank_df2t_init(resonator_bank_df1t_t *bank)
{
    if (bank == NULL) return -EINVAL;
    memset(bank, 0, sizeof(resonator_bank_df1t_t));
...
void resonator_bank_df2t_process(resonator_bank_df1t_t *bank, const int16_t *input, size_t len)
{
    /* Q15 → float [-1.0, 1.0] */
